...
namespace GGG.Tool
{
    public class DevelopmentToos
    {
        ...
        {
            UnityEngine.Debug.LogFormat($"日志内容:<color=#66ccff> --->   {message}   <--- </color>");
        }
        ...
        /// 将图片加载到 image 组件上
        /// </summary>
        public static void LoadTextureOnImage(string imagePath, Image image)
        {
            Texture2D t = Resources.Load<Texture2D>(imagePath);
        ...
        /// <returns></returns>
        public static bool EqualsFloat(float a, float b, float limit = 0.01f)
        {
            float sub = a - b;
        ...
        /// <returns></returns>
        public static bool IsSameLayerMask(GameObject obj, LayerMask mask)
        {
            return (mask.value & (1 << obj.layer)) != 0;
        }

        public static bool IsSameLayerMask(int layer, LayerMask mask)
        {
        ...
        /// <returns></returns>
        public static Vector3 GetTargetPoint(Transform enemy)
        {
            Collider enemyCollider = enemy.GetComponentInChildren<Collider>();
            if (enemyCollider)
                return enemyCollider.bounds.center;

            return enemy.position + Vector3.up * 0.5f;
        }

        public static Vector3 GetTargetPoint(Collider enemyCollider)
        {
            return enemyCollider.bounds.center;
        }
    }
